...
{
    Book* ans = new Book(categoryID(), prodName_, price_, qty_, author_, isbn_);
    return ans;
}
...
{
  Clothing* ans = new Clothing(categoryID(), prodName_, price_, qty_, size_, brand_);
  return ans;
}
...
{
  Movie* ans = new Movie(categoryID(), prodName_, price_, qty_, genre_, rating_);
  return ans;
}
